...
class PostReceiveTest extends CakeTestCase {
	...
	function testPushNewBranch() {
		...
		$this->assertEqual('master', $result[0]['Branch'][0]['name']);
		$this->assertTrue(empty($result[0]['Branch'][1]['name']));
		...
		$expected = 'Initial Project Commit';
		$this->assertEqual($expected, $result[0]['Commit']['message']);
		$this->assertEqual('new', $result[0]['Branch'][0]['name']);
		$this->assertEqual('master', $result[0]['Branch'][1]['name']);
	...
	function testPushSingle() {
		...
		$this->assertEqual($expected, $result[0]['Commit']['message']);
		$this->assertEqual(1, $result[0]['Branch'][0]['BranchCommit']['user_id']);
	...
	function testPushMultipe() {
		...
		$this->assertEqual($expected, $result[0]['Commit']['message']);
		$this->assertEqual(1, $result[0]['Branch'][0]['BranchCommit']['user_id']);
		$this->assertEqual(0, $result[0]['Branch'][0]['BranchCommit']['push_id']);

		$expected = 'Second Commit';
		$this->assertEqual($expected, $result[1]['Commit']['message']);
		$this->assertEqual(1, $result[1]['Branch'][0]['BranchCommit']['user_id']);
		$this->assertEqual($newrev, $result[1]['Branch'][0]['BranchCommit']['push_id']);
