...
function getStartingLineNumber(block){var classes=block.className.split(/\s+/);for(var i=0;i<classes.length;i++){if(classes=='no-linenumbers'){return-1;}
if(classes.match("startAt")!=null){return classes.match(/\d+/)/1;}}
if(lineNumbers)return 1;return-1;}
function getAlternatingRowsOn(block){var classes=block.className.split(/\s+/);for(var i=0;i<classes.length;i++){if(classes=='no-alternating-rows'){return false;}
if(classes=='alternating-rows'){return true;}}
return alternatingRows;}
var language='';function highlightBlock(block){try{var text=blockText(block);language=blockLanguage(block);}catch(e){if(e=='No highlight')
return;}
if(language){var result=highlight(language,text).value;}else{var max_relevance=2;var relevance=0;for(var key in selected_languages){if(!selected_languages.hasOwnProperty(key))
continue;var r=highlight(key,text);relevance=r.keyword_count+r.relevance;if(relevance>max_relevance){max_relevance=relevance;var result=r.value;language=key;}}}
if(result){if(startAt!=-1||alternatingRows){result=insertLines(result,startAt,alternatingRows);}
var className=block.className;if(!className.match(language)){className+=' '+language;}
result='<code class="'+className+'">'+result+'</code>';return result;}}
function replaceChildOnStage(block,result){var container=document.createElement('div');container.innerHTML='<pre>'+result+'</pre>';var environment=block.parentNode.parentNode;environment.replaceChild(container.firstChild,block.parentNode);}
function rjust(st,siz){t=siz-st.length;if(t>0){st=rspaces(t)+st;}
return st;}
function rspaces(len){s=" ";for(i=0;i<len;i++){s=s+" ";}
return s;}
function insertLines(highlightedText,startAt,alternatingRows){var counter=startAt+1-1;var result="<ol>";highlightedText=highlightedText.replace(/(\r\n|\r|\n)(\r\n|\r|\n)/g,"$1 $2");var lines=highlightedText.split(/\r\n|\r|\n/);var spaces=Math.ceil(Math.log(lines.length)/Math.log(10));var newline="";var line;var i;var tokenArray=new Array();if(spaces<6)
spaces=6;for(line=0;line<lines.length-1;++line){newline="";if(alternatingRows){newline="<span class=\"";if(line%2==0)
newline=newline+"coderow";else
newline=newline+"alternaterow";newline=newline+"\">";}
if(startAt!=-1){var paddingLeft=spaces-(Math.ceil(Math.log(counter+1)/Math.log(10)));newline=newline+"<a href=\"#"+counter+"\" name=\""+counter+"\" class=\"rownumber\">"+rspaces(paddingLeft)+counter+"</a>";}
for(var restartTokenCt=0;restartTokenCt<tokenArray.length;++restartTokenCt){newline=newline+tokenArray[restartTokenCt];}
var tokens=lines[line].match(/(<span class="\w+">)|(<\/span>)|(.*?)/g);for(var tokenct=0;tokenct<tokens.length;++tokenct){if(tokens[tokenct].match(/(<span class="\w+">)/)){tokenArray.push(tokens[tokenct]);}else if(tokens[tokenct].match(/(<\/span>)/)){tokenArray.pop();}}
newline=newline+lines[line];for(var restartTokenCt=0;restartTokenCt<tokenArray.length;++restartTokenCt){newline=newline+"</span>";}
newline=newline+"\n";if(alternatingRows){newline=newline+"</span>";}
counter++;result=result+newline;}
return result+"</ol>";}
...
function injectScripts(languages){var scripts=document.getElementsByTagName('SCRIPT');for(var i=0;i<scripts.length;i++){if(scripts[i].src.match(/ghighlight\.js(\?.+)?$/)){var path=scripts[i].src.replace(/ghighlight\.js(\?.+)?$/,'');break;}}
...
this.LANGUAGES=LANGUAGES;this.ALL_LANGUAGES=ALL_LANGUAGES;this.initHighlightingOnLoad=initHighlightingOnLoad;this.initHighlighting=initHighlighting;this.highlightBlock=highlightBlock;this.findCode=findCode;this.injectScripts=injectScripts;var lineNumbers=false;var startAt=-1;this.noLineNumbers=function(){startAt=-1;lineNumbers=false;}
this.addLineNumbers=function(){startAt=1;lineNumbers=true;}
var alternatingRows=false;this.noAlternatingRows=function(){alternatingRows=false;}
this.addAlternatingRows=function(){alternatingRows=true;}
this.IDENT_RE='[a-zA-Z][a-zA-Z0-9_]*';this.UNDERSCORE_IDENT_RE='[a-zA-Z_][a-zA-Z0-9_]*';this.NUMBER_RE='\\b\\d+(\\.\\d+)?';this.C_NUMBER_RE='\\b(0x[A-Za-z0-9]+|\\d+(\\.\\d+)?)';this.APOS_STRING_MODE={className:'string',begin:'\'',end:'\'',illegal:'\\n',contains:['escape'],relevance:0};this.QUOTE_STRING_MODE={className:'string',begin:'"',end:'"',illegal:'\\n',contains:['escape'],relevance:0};this.BACKSLASH_ESCAPE={className:'escape',begin:'\\\\.',end:'^',relevance:0};this.C_LINE_COMMENT_MODE={className:'comment',begin:'//',end:'$',relevance:0};this.C_BLOCK_COMMENT_MODE={className:'comment',begin:'/\\*',end:'\\*/'};this.HASH_COMMENT_MODE={className:'comment',begin:'#',end:'$'};this.C_NUMBER_MODE={className:'number',begin:this.C_NUMBER_RE,end:'^',relevance:0};}();var initHighlightingOnLoad=hljs.initHighlightingOnLoad;hljs.initHighlightingOnLoad();$(document).ready(function(){$("pre > code").parent().before("<span class=\"plain\"><a href=\"#plain\">plain</a></span>"
+" | <span class=\"highlight\"><a href=\"#highlight\">highlight</a></span>"
+" | <span class=\"numbers\"><a href=\"#numbers\">line numbers</a></span>");if(location.hash=='#highlight'){$('body').append('<div id="curtain">Can make pretty teh code?</div>');setTimeout(function(){hljs.noAlternatingRows();hljs.noLineNumbers();$(".highlighted").each(function(i){$(this).html(hljs.highlightBlock(code[i]));});$('#curtain').fadeOut(3,function(){$(this).remove();});},0);}else if(location.hash){$('body').append('<div id="curtain">Can make teh pretty code?</div>');setTimeout(function(){hljs.addAlternatingRows();hljs.addLineNumbers();$(".highlighted").each(function(i){$(this).html(hljs.highlightBlock(code[i]));});$('#curtain').fadeOut(3,function(){$(this).remove();});var line=location.hash.slice(1)-4;target=$("a[name="+line+"]");if(target.length){$("html,body").animate({scrollTop:target.offset().top},1000);}},0);}
var code=[];$("pre").each(function(i){$(this).addClass("highlighted");code.push(hljs.findCode(this));});$("span.plain > a").bind("click",function(){$('body').append('<div id="curtain">Speedy Gonzales..</div>');setTimeout(function(){$(".highlighted").each(function(i){$(this).html("<code>"+$(code[i]).html()+"</code>");});$('#curtain').fadeOut(3,function(){$(this).remove();});},0);});$("span.highlight > a").bind("click",function(){$('body').append('<div id="curtain">Time for a break? This could take a while...</div>');setTimeout(function(){hljs.noAlternatingRows();hljs.noLineNumbers();$(".highlighted").each(function(i){$(this).html(hljs.highlightBlock(code[i]));});$('#curtain').fadeOut(3,function(){$(this).remove();});},0);});$(".numbers a").bind("click",function(){$('body').append('<div id="curtain">Grab some coffee? This could take a while...</div</div>');setTimeout(function(){hljs.addAlternatingRows();hljs.addLineNumbers();$(".highlighted").each(function(i){$(this).html(hljs.highlightBlock(code[i]));});$('#curtain').fadeOut(3,function(){$(this).remove();});},0);});});
hljs.IDENT_RE_RU='[a-zA-Zа-яА-Я][a-zA-Z0-9_а-яА-Я]*';hljs.OneS_KEYWORDS={'процедура':1,'функция':1,'экспорт':1,'перем':1,'конецфункции':1,'конецпроцедуры':1,'если':1,'тогда':1,'иначе':1,'иначеесли':1,'конецесли':1,'попытка':1,'исключение':1,'конецпопытки':1,'ложь':1,'истина':1,'неопределено':1,'и':1,'или':1,'не':1,'null':1,'для':1,'каждого':1,'из':1,'по':1,'цикл':1,'конеццикла':1};hljs.LANGUAGES['1c']={defaultMode:{lexems:[hljs.IDENT_RE_RU],contains:['comment','string','function','preprocessor','number'],keywords:hljs.OneS_KEYWORDS},case_insensitive:true,modes:[hljs.C_LINE_COMMENT_MODE,{className:'string',begin:'"',end:'"',contains:['dquote'],relevance:0},{className:'string',begin:'"',end:'$',contains:['dquote']},{className:'string',begin:'\\|',end:'$',contains:['dquote']},{className:'string',begin:'\\|',end:'"',contains:['dquote']},{className:'dquote',begin:'""',end:'^'},{className:'number',begin:hljs.NUMBER_RE,end:'^',relevance:0},{className:'title',lexems:[hljs.IDENT_RE_RU],begin:hljs.IDENT_RE_RU,end:'^'},{className:'params',begin:'\\(',end:'\\)',lexems:[hljs.IDENT_RE_RU],keywords:{'знач':1},contains:['string']},{className:'function',begin:'(процедура|функция)',end:'$',lexems:[hljs.IDENT_RE_RU],keywords:{'процедура':1,'экспорт':1,'функция':1},contains:['title','tail','comment'],relevance:0},{className:'tail',begin:'^',endsWithParent:true,lexems:[hljs.IDENT_RE_RU],contains:['params','export']},{className:'export',begin:'экспорт',endsWithParent:true,lexems:[hljs.IDENT_RE_RU],keywords:{'экспорт':1},contains:['comment']},{className:'preprocessor',begin:'#',end:'$',lexems:[hljs.IDENT_RE_RU]}]};
...
hljs.LANGUAGES.bash={defaultMode:{lexems:[hljs.IDENT_RE],contains:['string','shebang','comment','number','test_condition','string','variable'],keywords:{'keyword':{'if':1,'then':1,'else':1,'fi':1,'for':1,'break':1,'continue':1,'while':1,'in':1,'do':1,'done':1,'echo':1,'exit':1,'return':1,'set':1,'declare':1},'literal':hljs.BASH_LITERAL}},case_insensitive:false,modes:[{className:'shebang',begin:'(#!\\/bin\\/bash)|(#!\\/bin\\/sh)',end:'^',relevance:10},hljs.HASH_COMMENT_MODE,{className:'test_condition',begin:'\\[ ',end:' \\]',contains:['string','variable','number'],lexems:[hljs.IDENT_RE],keywords:{'literal':hljs.BASH_LITERAL},relevance:0},{className:'test_condition',begin:'\\[\\[ ',end:' \\]\\]',contains:['string','variable','number'],lexems:[hljs.IDENT_RE],keywords:{'literal':hljs.BASH_LITERAL}},{className:'variable',begin:'\\$([a-zA-Z0-9_]+)\\b',end:'^'},{className:'variable',begin:'\\$\\{(([^}])|(\\\\}))+\\}',end:'^',contains:['number']},{className:'string',begin:'"',end:'"',illegal:'\\n',contains:['escape','variable'],relevance:0},{className:'string',begin:'"',end:'"',illegal:'\\n',contains:['escape','variable'],relevance:0},hljs.BACKSLASH_ESCAPE,hljs.C_NUMBER_MODE,{className:'comment',begin:'\\/\\/',end:'$',illegal:'.'}]};
